...
CHAN_default=['HHZ' , 'HHN' , 'HHE' , 'BHZ' , 'BHN' , 'BHE' , 'LHZ' , 'LHN' , 'LHE','HLZ' , 'HLN' , 'HLE' , 'BLZ' , 'BLN' , 'BLE' , 'LLZ' , 'LLN' , 'LLE']

argu_parser = argparse.ArgumentParser(description='extract one hour long segments from FILES and store them in the PATH_SDS directory. Use options -s -n and -l to force the station, network and location code in mseed files. Otherwise the orginal informations are preserved. Directory''s name of the SDS is based on these codes. ')
argu_parser.add_argument("-f", "--files", required=True, help="File(s) to be read. If it contains wildcards(*,?,...), the string must be quoted. Example : python extract2sds.py -s TOTO -n XX \"../*HH?.*\"")
argu_parser.add_argument("-c","--channels", nargs='+', default=CHAN_default, help="Process only CHANNELS. Do not use this option if you want to process all available channels for the corresponding station. Separate CHANNELS with spaces. No wildcard. Default is all LH?,BH?,HH?")
argu_parser.add_argument("-b", "--starttime", default=UTCDateTime(2015,9,12),type=UTCDateTime, help="Start time for processing. Various format accepted. Example : 2012,2,1 / 2012-02-01 / 2012,032 / 2012032 / etc ... See UTCDateTime for a complete list. Default is 2010-1-1")
argu_parser.add_argument("-e", "--endtime", default=UTCDateTime(3000,1,1),type=UTCDateTime, help="End time for processing. Various format accepted. Example : 2012,2,1 / 2012-02-01 / 2012,032 / 2012032 / etc ... See UTCDateTime for a complete list. Default is 2015-1-1")
argu_parser.add_argument("-s" ,"--station", help="set station code")
argu_parser.add_argument("-n" ,"--network", help="set network code")
argu_parser.add_argument("-l" ,"--location", help="set location code. \"\" if none ")
argu_parser.add_argument("-o", "--path_sds", default=PATH_SDS,help="Base directory for the SDS. Default is "+PATH_SDS+" (default path can be modified in default_qc_path)")
...
try : 
	...
	for y in year_dir:
		...
		for n in net_dir:
			...
			for s in sta_dir:
				...
				for chan in CHAN:
					try : 
						os.mkdir(PATH_SDS+str(y)+'/'+n+'/'+s+'/'+chan+".D")
					except:
						pass
	...
	for day in Days:
		Stream=read(Files,starttime=day,endtime=day+86400,nearest_sample=False)
		for chan in CHAN:
			...
			for tr in S:
				...
				nameout="%s/%04i/%s/%s/%s.D/%s.%s.%s.%s.D.%04i.%03i" % 	(PATH_SDS,t0.year,net,sta,chan,sta,net,chan,locid,t0.year,t0.julday)
				tr.write(nameout,format="MSEED")
				print("write %s (%i samples, %10.2f seconds)" % (nameout,tr.stats.npts,tr.stats.endtime-t0))
