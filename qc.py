...
#                return False
        ...
        tr.simulate(paz_remove=paz, remove_sensitivity=True, paz_simulate=None, simulate_sensitivity=False)

        # Spike detector
        # band-pass filtering
        tr_filt = bandpass (tr.data,DP_f_min,DP_f_max,self.sampling_rate,corners=DP_f_corners)
        # STA/LTA
        #cft = recStaltaPy(tr_filt,DP_sta*self.sampling_rate,DP_lta*self.sampling_rate)
        cft = recursive_sta_lta(tr_filt,int(DP_sta*self.sampling_rate),int(DP_lta*self.sampling_rate))
    
        #on_of = triggerOnset(cft,DP_thres_high,DP_thres_low,max_len=DP_max_len*self.sampling_rate)
        on_of = trigger_onset(cft,DP_thres_high,DP_thres_low,max_len=DP_max_len*self.sampling_rate)
    ...
    def get_percentile(self, percentile=50, hist_cum=None, time_lim=(T1,T2)):
        ...
        """
        if hist_cum is None:
            hist_cum = self.__get_normalized_cumulative_histogram(time_lim=time_lim)	    
        # go to percent
...
# -------------------- P L O T -----------------------------------------
	...
	# COVERAGE
        ...
        # plot data coverage
        starts = date2num(times_used);ends = date2num(times_used+PPSD_LENGTH)
        for start, end in zip(starts, ends):
            ax_coverage.axvspan(start, end, 0, 0.7, alpha=0.5, lw=0)
        # plot data really available
	aa=[(start,end) for start, end in self.times_data if ((end-start)>PPSD_LENGTH)] # avoid very small gaps	otherwise very long to plot
        for start, end in aa:
            start = date2num(start.datetime)
            end = date2num(end.datetime)
            ax_coverage.axvspan(start, end, 0.7, 1, facecolor="g", lw=0)
        # plot gaps
	aa=[(start,end) for start, end in self.times_gaps if ((end-start)>PPSD_LENGTH)] # avoid very small gaps	otherwise very long to plot
        for start, end in aa:
            start = date2num(start.datetime)
            end = date2num(end.datetime)
	...
	t = date2num(times_used)
	f = 1./self.per_octaves
        T, F = np.meshgrid(t , f)
	spectro = ax_spectrogram.pcolormesh(T,F,transpose(psd))
	spectro.set_clim(*psd_db_limits)
	
	cb = colorbar(spectro, cax=ax_col_spectrogram, orientation='horizontal',ticks=linspace(psd_db_limits[0],psd_db_limits[1],5),format='%i')
	cb.set_label("dB")	
	...
	for i,h in enumerate(Hour):
		...
		A=ma.masked_array(a,mask=~((a >psdh_db_limits[0]) & (a <psdh_db_limits[1])))
		psdH[i,:]=ma.getdata(A.mean(axis=0))
	psdH=array([psdH[:,i]-psdH[:,i].mean() for i in arange(0,psdH.shape[1])])
	H24, F = np.meshgrid(Hour , f)
	spectroh = ax_spectrogramhour.pcolormesh(H24,F,psdH, cmap=cm.RdBu_r)
	spectroh.set_clim(-8,8)
	
	cb = colorbar(spectroh, cax=ax_col_spectrogramhour, orientation='horizontal',ticks=linspace(-8,8,5),format='%i')
	cb.set_clim(-8,8)
	...
	for start, end in zip(starts_uncov, ends_uncov):
		ax_spectrogram.axvspan(start, end, 0, 1, facecolor="w", lw=0 , zorder=100)
		ax_freqpsd.axvspan(start, end, 0, 1, facecolor="w", lw=0 , zorder=100)
		ax_spikes.axvspan(start, end, 0, 1, facecolor="w", lw=0 , zorder=100)
		
	# LEGEND
	leg=[str(xx)+'-'+str(yy)+' s' for xx,yy in zip(per_left,per_right )]
	hleg=ax_freqpsd.legend(leg,loc=3,bbox_to_anchor=(-0.015,0.75),ncol=size(leg))
	...
	# PPSD	
        ...
        if show_percentiles:
		hist_cum = self.__get_normalized_cumulative_histogram(time_lim=(starttime,endtime))
		# for every period look up the approximate place of the percentiles
		for percentile in percentiles:
			periods, percentile_values = self.get_percentile(percentile=percentile,hist_cum=hist_cum,time_lim=(starttime,endtime))
...
def main():
    ...
    # Arguments
    argu_parser = argparse.ArgumentParser(description="Run the main code to compute quality sheets for a set of STATIONS")
    argu_parser.add_argument("-s","--stations", nargs='+', required=True, help="list of stations name. Stations must be in the station_dictionnary file. Separate STATIONS with spaces")
    argu_parser.add_argument("-b", "--starttime", default=UTCDateTime(2000,1,1),type=UTCDateTime, help="Start time for processing. Various format accepted. Example : 2012,2,1 / 2012-02-01 / 2012,032 / 2012032 / etc ... See UTCDateTime for a complete list. Default is 2010-1-1")
    argu_parser.add_argument("-e", "--endtime", default=UTCDateTime(2055,9,16),type=UTCDateTime, help="End time for processing. Various format accepted. Example : 2012,2,1 / 2012-02-01 / 2012,032 / 2012032 / etc ... See UTCDateTime for a complete list. Default is 2015-1-1")
    argu_parser.add_argument("-c","--channels", nargs='+', help="Process only CHANNELS. Do not use this option if you want to process all available channels indicated in station_dictionnary. Separate CHANNELS with spaces. No wildcard. Default is all channels")
    argu_parser.add_argument("-pkl", "--path_pkl", default=PATH_PKL,help="output directory for pkl files. Default is "+PATH_PKL+" (defined in default_path_qc.py file) ")
    argu_parser.add_argument("-plt", "--path_plt", default=PATH_PLT,help="output directory for plt files. Default is "+PATH_PLT+" (defined in default_path_qc.py file) ")
    argu_parser.add_argument("-nb_days_pack", type=int, default="7",help="To avoid to load too much data : load NB_DAYS_PACK days before computing the psd+detection+plot. NB_DAYS_PACK should be strictly higher than 1 day. Default is 7 days")
    argu_parser.add_argument("-nb_max_traces", type=int, default="100",help="Maximum number of traces by Stream after trimming. To avoid too long computations caused by too much gaps. Default is 100") 
    argu_parser.add_argument("-force_paz", default=False, action='store_true', help="Use this option if you want don't want to use the dataless file specified in station_dictionnary. Only PAZ response computed from sensor and digitizer will be used. Use for debug only. Dataless recommended") 
    ...
    stop=args.endtime
    # Processing options
    nb_days_pack=args.nb_days_pack # To avoid to load too much data : load nb_days_pack days before computing the psd+detection
    nb_max_traces=args.nb_max_traces # Maximum number of traces by Stream (nb_days_pack long) afterter trim. To avoid too long computations
    # Output Paths
    ...
    for sta in STA:
        ...
        for chan in chan_proc:      
            ...
            for d in Days:
                d_str="%03d" % (d.julday)
                y_str="%04d" % (d.year)
                ID={'sta':sta , 'locid':locid , 'net':net , 'chan':chan , 'year':y_str , 'day':d_str}
                ...
                try :       
                    ...
                    if is_pickle==False :
                        tr=stream[0]
                        S=QC(tr.stats,parser=parser,paz=paz,skip_on_gaps=True)
                        is_pickle=True
                    # Add traces
                    if nb_days_read==1 :
                        Stream=stream
                ...
                if ((nb_days_read>=nb_days_pack) or (d==Days[-1])) and Stream:              
                    # Remove the minutes before the next hour (useful when computing statistics per hour)
                    mst=min([temp.stats.starttime for temp in Stream]) # mst = min start time
                    mst=UTCDateTime(mst.year,mst.month,mst.day,mst.hour,0,0)+3600.              
                    ...
                    # save
                    # This can be 2 levels below to save a little bit of time (save and loading)
                    if is_pickle :
                        print filename_pkl
                        S.save(filename_pkl)
                    ...
                    if is_pickle and len(S.times_used)>0 :
                        filename_plt=PATH_PLT+'/'+net+"."+sta+"."+locid+"."+chan+".png"
