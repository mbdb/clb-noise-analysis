...
for sta in STA:
	...
	for chan in CHAN:		
		...
		try : 	
			...
			bool_times_select=(times_used>starttime1) & (times_used<endtime1)
			times_used1=times_used[bool_times_select]
			times_data1=times_data[(times_data[:,0]>starttime1) & (times_data[:,1]<endtime1)]
			psd1=S.psd[bool_times_select,:]
			mpsd1=median(psd1,axis=0)
			spikes1=S.spikes[bool_times_select]								

			bool_times_select=(times_used>starttime2) & (times_used<endtime2)
			times_used2=times_used[bool_times_select]
			times_data2=times_data[(times_data[:,0]>starttime2) & (times_data[:,1]<endtime2)]
			...
			psdH1=array([array(psd1[HourUsed1==h,:]).mean(axis=0) for h in Hour])
			spH1=array([array(spikes1[HourUsed1==h,:]).mean(axis=0) for h in Hour])
			HourUsed2=array([t.hour for t in times_used2])
			psdH2=array([array(psd2[HourUsed2==h,:]).mean(axis=0) for h in Hour])
			spH2=array([array(spikes2[HourUsed2==h,:]).mean(axis=0) for h in Hour])			
			...
			# coverage			
				...
				ax_coverage_1.axhspan(start.datetime, end.datetime, color='k', lw=0)

		        starts2 = date2num(times_used2);ends2 = date2num(times_used2+S.len)
		        for start, end in times_data2:
				ax_coverage_2.axhspan(start.datetime, end.datetime, color='m', lw=0)
			...
			ax_coverage_1.set_ylabel("Time 1 (reference) = %5.1f days (black)" % (diff(times_data1).sum()/86400))
			ax_coverage_2.set_ylabel("Time 2 = %5.1f days (magenta)" % (diff(times_data2).sum()/86400))
			ax_coverage_1.yaxis.set_label_position('right')
			ax_coverage_2.yaxis.set_label_position('right')
			
			# PSD
			ax_mpsd.fill_between(S.period_bins,mpsd1,y2=mpsd2,where=(mpsd1>mpsd2),color='b',alpha=0.5)
			ax_mpsd.fill_between(S.period_bins,mpsd1,y2=mpsd2,where=(mpsd1<mpsd2),color='r',alpha=0.5)
			...
			ax_diff_mpsd.fill_between(S.period_bins,mpsd2-mpsd1,y2=0,where=(mpsd2>mpsd1),color='r',alpha=0.5)
			ax_diff_mpsd.fill_between(S.period_bins,mpsd2-mpsd1,y2=0,where=(mpsd2<mpsd1),color='b',alpha=0.5)
			...
			cb = colorbar(h, cax=ax_colorbar_diff_mpsd_hour, orientation='horizontal',ticks=linspace(-8,8,5),format='%i')			
			...
			ax_diff_spikes_hour.fill_betweenx(Hour,spH2-spH1,x2=0,where=(spH2<spH1),color='b',alpha=0.5)
			ax_diff_spikes_hour.fill_betweenx(Hour,spH2-spH1,x2=0,where=(spH2>spH1),color='r',alpha=0.5)			
