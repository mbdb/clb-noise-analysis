...
try :
	...
	#PSD
	ax_mpsd.errorbar(S1.period_bins,mpsd1,yerr=stdmpsd1, color='k' , linewidth=2, ecolor='k' , elinewidth=0.2)
	ax_mpsd.errorbar(S1.period_bins,mpsd2,yerr=stdmpsd2, color='m' , linewidth=2, ecolor='m' , elinewidth=0.2)
	ax_mpsd.fill_between(S1.period_bins,mpsd1,y2=mpsd2,where=(mpsd1>mpsd2),color='b',alpha=0.5)
	ax_mpsd.fill_between(S1.period_bins,mpsd1,y2=mpsd2,where=(mpsd1<mpsd2),color='r',alpha=0.5)
	# Noise models
	...
	ax_mpsd.set_title("station %s (magenta) - station %s (black)" % (S2.id,S1.id))
	...
	# Spikes diff
	ax_diff_spikes.plot(TPnum[Spikesdiff>0],Spikesdiff[Spikesdiff>0],color='r',marker='.',linestyle='None',alpha=0.5)
	ax_diff_spikes.plot(TPnum[Spikesdiff<=0],Spikesdiff[Spikesdiff<=0],color='b',marker='.',linestyle='None',alpha=0.5)	
	...
	ax_diff_spikes_hour.fill_between(Hour,spH2-spH1,y2=0,where=(spH2<spH1),color='b' , alpha=0.5)
	ax_diff_spikes_hour.fill_between(Hour,spH2-spH1,y2=0,where=(spH2>spH1),color='r' , alpha = 0.5)	
