...
STA=['MYY' , 'POLI' , 'ROVI' , 'CAVD' , 'URBES' , 'BARI2' , 'BARI3' ,  'VAUD' , 'CRUG' , 'VERTU' , 'GPD' , 'SAVO' , 'SACE' , 'GRN5' , 'OGMO' , 'ROST' , 'NEUF']
...
allow_above_class=3. #dB above class limit to accept a site as A or B (useful if PSd median is just about the class limit in a restricted period range
...
Per=logspace(log10(0.1),log10(200),100) # should be modified acording to Per_sort and freq sampling
...
for comp in COMP:
	...
	Times_used=[UTCDateTime(t.year,t.month,t.day,t.hour) for t in arange(period[0],period[1],3600)]	
	for ista,sta in enumerate(STA):	
		...
		try : 	
			...
			try :
				periods,psd=S.get_percentile(percentile=50, hist_cum=None, time_lim=period)
			except :
				print S.id+" have no data in the time range"
			else :		
				finterp=interpolate.interp1d(periods,psd,bounds_error=False)			
	...
	for per_sort in Per_sort:
		...
		ax_psd.set_title("PSD - NLNM / Comp %s / sorted by T=[%4.1f - %4.1f] s" % (comp,per_sort[0],per_sort[-1]))
		cb = colorbar(h, cax=ax_colorbar, orientation='horizontal',ticks=arange(Clim[0],Clim[1]+1,10),format='%i')
		...
		if show_class:
			...
			if show_txt_class:			
				for i,aa in enumerate(PSDs[:,iper_sort[0]:iper_sort[-1]]):				
					isA=np.all(aa<(PSDclass[0,iper_sort[0]:iper_sort[-1]]+allow_above_class))
					if isA:
						txtclass=" A / "
					else:
						txtclass=" - / "
		...
		savefig(PATH_PLT_OUT+basename_output+"_%s_%4.1f_%4.1f.png" % (comp,per_sort[0],per_sort[-1]))
